...
static ssize_t spi_flash_nor_write_reg(struct spi_nor *nor, u8 opcode, const u8 *buf,
			     size_t len)
{
	/* Write WRITE ENABLE command*/
	write_command(nor, WRITE_ENABLE);
...
static const struct spi_nor_controller_ops litex_spi_flash_controller_ops = {
...
static int litex_spi_flash_probe(struct platform_device *pdev)
{
	...
	nor->controller_ops = &litex_spi_flash_controller_ops;
	/*
